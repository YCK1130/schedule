...
/**
 * 檢查一個 GroupRestriction 條件是否符合
 * 支援範圍限制 (minCount ~ maxCount)
 */
// const checkGroupRestriction = (count: number, restriction: GroupRestriction): boolean => {
//     // 如果存在最小值和最大值，則視為範圍限制
//     if (restriction.minCount !== undefined && restriction.maxCount !== undefined) {
//         return count >= restriction.minCount && count <= restriction.maxCount;
//     }
 ...
 * 從多個限制條件中找出適合特定職位的條件
 */
const getRestrictionRange = (restrictions: GroupRestriction[], position: string = "所有"): { min: number; max: number } => {
    // 尋找匹配職位的限制條件
        ...
        const optimizedGreedyMatching = () => {
            ...
            for (const timeSlot of allTimeSlots) {
                ...
                const availableInterviewees = validInterviewees.filter(
                    (interviewee) => isTimeSlotAvailable(interviewee, timeSlot, interviews) && !unmatched.interviewees.includes(interviewee)
                );
